...
def mcmc_routine(ndim, N_walkers, theta, params_T, params_indices, fix_params, k_range, Pk_wnow_obs, icov_Pk_wnow, Pwig_spl, Psm_spl, norm_gf, params_name, pool):
    ...
    return mergedsamples, np.array(theta_mcmc)
...
def fit_BAO():
    ...
    parser.add_argument("--nSV_min", help = "Fitting the extracted power spectrum with nSV used. nSV_min is the strating point.", type=int, default=1)
    parser.add_argument("--save_sampler", help = "Whether we save samplers of MCMC into a data file or not. True or False.", default='False')
    parser.add_argument("--modify_Cov_cij_cpq", help = "Whether we have modified the Cov_cij_cpq while outputing Cijl^prime and Gm^prime.", default='False')
    ...
    if rank == 0:
        if not os.path.exists(odir):
            ...
            if args.save_sampler == 'True':
                os.makedirs(odir + 'chain_samplers/')        # store positions of samplers in chains
    comm.Barrier()
    ...
    for num_eigv in range(args.nSV_min, num_kbin+1):
        ...
        if (args.save_sampler == 'True') and (rank == 0):
            ...
            ofile = odir_merged_sample + 'mergedsamples_' + filename[0: len(filename)-4] + '.npz'
