...
# Copy the same code from ../, import the lens_eff from lens_eff_module in the folder KW_modules. --06/18/2018
# 1. Modify z_array, extend the maximum value to z_max. --07/09/2018
#
# -----------------------------------------------------------------------
...
parser.add_argument("--survey_stage", help = '*Survey stage, e.g., KW_stage_IV/precise_Cijl_Gm', required=True)
...
def main():
    ...
    eps = 1.e-12                           # small number as an x-axis boundary to calculate lens efficiency g^i or not
    ...
    z_array = np.linspace(0.0, z_max, 1000, endpoint=False)
    ...
    ofile = odir + 'gi_nrbin{}_zi_{}.pdf'.format(nrbin, z_target)
