...
#  Copy the code from mpi_KW_cross_convergence_ps_bin.py, modify the process calculating Cijl and G matrix. --09/12/2018
...
# 5. Modify the function plot_numd_spectroscopy. No need to take account of scale_n for dn(z)/dz plotting, since dn/dz has considered the total
#    angular density. --11/25/2018
#
...
def main():
    ...
    def Gm_integrand_out(k, c_i, spl_gi, spl_gj, ell):
        ...
        if (chibin[c_i+1]-1.e-8) < chi_k:
            ...
            return (1.0+z_k)**2.0* spl_gi(chi_k) * spl_gj(chi_k) * ell/k**2.0 * GF
            ...
            for j in range(num_kout):
                ...
                for i in range(N_dset):
                    ...
                    if chibin[rb_i+1] > ell/kout[j+1]:
                        ...
                        res = integrate.quad(Gm_integrand_out, kout[j], kout[j+1], args=(rb_i, spl_gi_list[rb_i], spl_gi_list[rb_j], ell), epsabs=1.e-6, epsrel=1.e-6)
