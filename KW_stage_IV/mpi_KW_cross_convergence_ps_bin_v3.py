...
# Copy the same code as mpi_KW_cross_convergence_ps_bin.py, add the additonal parameter alpha as the BAO scale shifting parameter. Same as the code (with the same name) in
# ./precise_Cijl_Gm/, we test whether the Fisher element from "rough" calculation of Cijl would resolve the issue of large value of Fisher element. --07/26/2018
#----------------------------------------------------------------------------------------------------------------
...
def main():
    ...
    odir = './BAO_alpha_{}/mpi_preliminary_data_{}/'.format(alpha, Pk_type)
    if rank == 0:
        if Psm_type == 'Pnow':
...
#######################################################
    ...
    def plot_numd_spectroscopy():
        ...
        ax.set_title("KW Stage-IV", fontsize=20)
