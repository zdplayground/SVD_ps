...
# ---------- Copy the code from ../precise_Gm. -- 04/29/2018
# Copy the same code from ../, modify it to distinguish the input directories of C^{ij}(l) and G matrix. We could use the same
# C^{ij}(l) in the folder ../. -- 04/23/2018
# -------------- Copy the same code from the folder ../pseudo_KW_stage_IV, modify the value of f_sky. --03/06/2018
...
parser.add_argument("--idir0_Cijl", help = "*The basic directory of input Cijl data files, e.g., './'.", required=True)
parser.add_argument("--idir0_Gm", help = "The basic directory of input G matrix, e.g., './'.")
parser.add_argument("--odir0", help = "*The basic directory of output files, e.g., './'.", required=True)
...
def cal_sn_dep_Cov_cij():
    ...
    idir_Gm = idir0_Gm + 'mpi_preliminary_data_Pwig_nonlinear/'
    ##idir = '/Users/ding/Documents/playground/shear_ps/SVD_ps/TF_cross-ps/' + 'preliminary_data_{}/'
    ...
    if rank == 0:
        ...
        inputf = idir_Gm + prefix + 'pseudo_shapenoise_{0}rbins.out'.format(num_rbin)   # read the default shape noise \sigma^2/n^i
    ...
    file_Gm_cross = idir_Gm + prefix + 'Gm_cross_out_{0}rbins_{1}kbins_CAMB.bin'.format(num_rbin, num_kout)
    ##file_Gm_cross = '/Users/ding/Documents/playground/shear_ps/SVD_ps/TF_cross-ps/preliminary_data_Pwig_nonlinear/' + prefix + 'Gm_cross_out_{0}rbins_{1}kbins_CAMB.bin'.format(num_rbin, num_kout)
