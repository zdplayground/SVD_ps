...
# 1. Modify the directory of cov_matrix_module. Output S/N instead of (S/N)^2. Input the number of ell for each rank from one file. --10/24/2018
#
...
parser.add_argument("--idir0", help = "*The basic directory of input files, e.g., './PW_stage_IV/BAO_alpha_1.0/'.", required=True)
parser.add_argument("--odir0", help = "*The basic directory of output files, e.g., './PW_stage_IV/BAO_alpha_1.0/'.", required=True)
...
def cal_signal_noise_ratio():
    ...
    if rank == 0:
        ...
        inputf = idir0 + 'mpi_preliminary_data_Pwig_nonlinear/' + prefix + 'pseudo_shapenoise_{0}rbins_ext.out'.format(red_bin_ext)
    ...
    ifile = '/Users/ding/Documents/playground/shear_ps/SVD_ps/Input_files/PW_stage_num_ell_per_rank_comm_size{}.dat'.format(size)  # num of ell is roughly estimated
